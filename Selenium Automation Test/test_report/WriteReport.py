...
workbook = xlsxwriter.Workbook('TestReport5.xlsx')
...
def init(worksheet):
    ...
    define_format_H1.set_bg_color("#00EC00")
    #define_format_H1.set_color("#00EC00")
    define_format_H2.set_align("center")
    #define_format_H2.set_bg_color("while")
    #define_format_H2.set_color("while")
    ...
    _write_center(worksheet, "B4", '运行环境', workbook)
    _write_center(worksheet, "B5", '客户端环境', workbook)
    _write_center(worksheet, "B6", '测试日期', workbook)
    ...
    _write_center(worksheet, "C6", data['test_date'], workbook)

    _write_center(worksheet, "D3", "用例总数", workbook)
    _write_center(worksheet, "D4", "通过总数", workbook)
    _write_center(worksheet, "D5", "失败总数", workbook)
    _write_center(worksheet, "D6", "错误总数", workbook)



    data1 = {"test_sum": 100, "test_success": 80, "test_failed": 5, "test_error": 5}
    ...
    _write_center(worksheet, "E6", data1['test_error'], workbook)

    _write_center(worksheet, "F3", "通过率", workbook)


    worksheet.merge_range('F4:F6', '80%', get_format_center(workbook))
...
def pie(workbook, worksheet):
   ...
   'values':    '=测试总况!$E$4:$E$6',
    ...
    chart1.set_title({'name': 'Web功能测试统计'})
    chart1.set_style(10)
